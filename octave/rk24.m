%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Written by Leon Brin                              11 June 2016 %
% Purpose: This function implements an adaptive rk2(4) method    %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [y,t,evals] = rk24(f,a,ya,b,h,eps,N)
  i = 1;
  t(i) = a;
  y(i) = ya;
  done = 0;
  ...
  while (!done && i<=N)
    ...
    k3 = f(t(i)+2*h/3, y(i)+h/3*(-k1+3*k2));
    k4 = f(t(i)+h,     y(i)+h*(k1-k2+k3));
    err = abs(h/8*(k1-k2-k3+k4));
    evals = evals+4;
    if (done || err<=eps)
